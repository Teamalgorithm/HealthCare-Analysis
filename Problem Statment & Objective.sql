...
--------------------------------------------------------------------------------------
-- Question 4: WHAT IS THE AVERAGE LENGTH OF HOSPITAL STAY FOR EACH MEDICAL CONDITION?
--------------------------------------------------------------------------------------
  
  -- PROBLEM STATEMENT:
     -- Determine the average duration of hospital stay for each medical condition
     -- to help hospitals plan resource allocation, bed management, and patient care.

  -- GOAL:
     -- Find the average length of stay for each medical condition, 
     -- enabling better resource planning, bed management, and patient care strategies.


SELECT Medical_Condition,
       AVG(DATEDIFF(DAY,Date_of_Admission,Discharge_Date)) AS Duration_Stay
FROM PatientsData_healthecare_clean
GROUP BY Medical_Condition;


-------------------------------------------------------------------------------
-- Question 5: WHICH MEDICATIONS ARE MOST COMMONLY PRESCRIBED FOR EACH DISEASE?
-------------------------------------------------------------------------------

  -- PROBLEM STATEMENT:
     -- Identify the most frequently prescribed medications for each medical condition 
     -- to understand treatment patterns and optimize pharmaceutical management.

  -- GOAL:
     -- Improve medication planning and ensure high-demand drugs are adequately stocked for effective treatment.

-- Count prescriptions and rank medications per disease
WITH MedicationCounts AS (
    SELECT 
        Medical_Condition,                    -- The disease 
        Medication,                           -- Medication prescribed
        COUNT(*) AS Prescription_Count,       -- Number of times this medication was prescribed
        ROW_NUMBER() OVER (
            PARTITION BY Medical_Condition    -- Separate ranking for each disease
            ORDER BY COUNT(*) DESC            -- Rank by prescription count 
        ) AS rn
    FROM PatientsData_healthecare_clean
    GROUP BY Medical_Condition, Medication    -- Group by disease and medication to count prescriptions
)
-- Select only the top-ranked medication for each disease
SELECT 
    Medical_Condition,                        -- Disease name
    Medication AS Most_Prescribed_Medication, -- The medication prescribed most often
    Prescription_Count                        -- Number of times it was prescribed
FROM MedicationCounts
WHERE rn = 1                                  -- Keep only the highest-ranked medication per disease
ORDER BY Medical_Condition;                   -- Sort output alphabetically by disease


-----------------------------------------------------------------
-- Question 6: HOW DO TEST RESULTS VARY BY DISEASE OR MEDICATION?
-----------------------------------------------------------------
    -- PROBLEM STATEMENT:
...
----------------------------------------------------------------
-- Question 7: WHICH HOSPITALS HAVE THE HIGHEST PATIENTS INTAKE?
----------------------------------------------------------------
    -- PROBLEM STATEMENT:
...
--------------------------------------------------------------------------------
-- Question 8: WHAT IS THE AVERAGE BILLING AMOUNT FOR PATIENTS AT EACH HOSPITAL? 
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------
-- Question 9: HOW DOES A DOCTOR'S PERFORMANCE VARY BASED ON PATIENT VOLUME OR TREATMENT OUTCOMES?
--------------------------------------------------------------------------------------------------
...
WITH DoctorPerformance AS (
    SELECT 
        ...
        SUM(CASE WHEN Test_Results = 'Unknown' THEN 1 ELSE 0 END) AS Unknown_results,
...
SELECT 
    ...
    Unknown_results,        -- Patients whose results were unknown
...
--------------------------------------------------------------------------------
-- Question 10: WHICH MEDICAL CONDITIONS DOES EACH DOCTOR TREAT MOST FREQUENTLY?
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- Question 11: WHAT IS THE COMMON ADMISSION TYPE PER HOSPITAL?
--------------------------------------------------------------------------------
...
---------------------------------------------------------------
-- Question 12: WHAT IS THE AVERAGE TREATMENT COST PER DISEASE?
---------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------------------------
-- Question 13: HOW DO INSURANCE PROVIDERS COMPARE IN TERMS OF THE NUMBER OF PATIENTS THEY COVER & TOTAL TREATMENT COST?
------------------------------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------------------
-- Question 14: WHICH MEDICAL CONDITIONS ARE ASSOCIATED WITH THE HIGHEST AND LOWEST HOSPITAL BILLS?
---------------------------------------------------------------------------------------------------

  -- Problem Statement:
     -- Compare medical conditions based on the number of patients, 
     -- and the highest, lowest, and average hospital billing amounts.

  -- Goal:
     -- To identify which medical conditions are associated with the highest 
     -- and lowest costs, and provide insight into cost distribution across conditions.


SELECT 
    Medical_Condition,                          
    COUNT(*) AS Patient_Count,                   -- Number of patients 
    ROUND(AVG(Billing_Amount),2) AS Avg_Billing,          -- Average billing amount per condition
    ROUND(MAX(Billing_Amount),2) AS Max_Billing,          -- Highest billing amount within that condition
    ROUND(MIN(Billing_Amount),2) AS Min_Billing           -- Lowest billing amount within that condition
FROM PatientsData_healthecare_clean               
GROUP BY Medical_Condition                            -- Group data so that each medical condition forms one summary row
ORDER BY Avg_Billing DESC;                            -- Sort results from highest to lowest average billing



------------------------------------------------------------------------
-- Question 15: WHAT ARE TOTAL HOSPITAL BILLING AMOUNS PER MONTH & YEAR?
------------------------------------------------------------------------
...
-------------------------------------------------------------------------
-- Question 16: WHAT IS THE YEARLY OCCUPANCY RATE FOR EACH HOSPITAL ROOM?
-------------------------------------------------------------------------

  -- Problem Statement:
       -- Determine the yearly occupancy rate for each hospital room by calculating 
       -- the total number of days each room was occupied relative to a full year.
       -- This helps to understand room utilization and identify underused or overused rooms.

  -- Goal:
       -- To track how efficiently each room is being used over the year, 
       -- monitor patterns of occupancy, and provide insights for staffing, 
       -- maintenance scheduling, and resource allocation.

-- Calculate the yearly occupancy rate for each hospital room
SELECT
    Room_Number,
    CAST(
        CASE 
            WHEN SUM(DATEDIFF(DAY, Date_of_Admission, Discharge_Date)) > 365 -- Calculate total occupied days by summing length of all patient stays
            THEN 100.0              -- If total occupied days > 365, cap at 100% (full year)
            ELSE SUM(DATEDIFF(DAY, Date_of_Admission, Discharge_Date)) / 365.0 * 100  -- Otherwise, divide total days by 365 to get fraction of year
        END AS DECIMAL(5,2)
    ) AS Occupancy_Rate_Percent
FROM PatientsData_healthecare_clean
GROUP BY Room_Number -- Aggregate per room
ORDER BY Room_Number;  -- Sort by room
...
SELECT
    Season,                       -- Season name
    COUNT(*) AS Patient_Count      -- Number of admissions in that season
FROM PatientsData_healthecare_clean
GROUP BY Season
ORDER BY CASE 
...
---------------------------------------------------------------------
-- Question 18: WHAT IS THE AVERAGE STAY DURATION BY ADMISSION TYPE?
---------------------------------------------------------------------

  -- Problem Statement:
     -- Determine the average length of hospital stay for patients based 
     -- on their admission type (e.g., emergency, elective, urgent) 
     -- to understand differences in patient care duration.

  -- Goal:
     -- To analyze how admission type affects the duration of hospitalization, 
     -- which can help in resource planning, bed management, and identifying 
     -- areas where efficiency or care protocols may be improved.
...
-----------------------------------------------------------------------
-- Question 19: year-over-year change in case volume
-----------------------------------------------------------------------
